...
DATA_DIR = "data"
MODELS_DIR = "models"
YENI_VERI_DIR = "data/yeni_gercek_veriler" # Canan'ın veritabanından gelecek gerçek verilerin klasörü

# Eğer klasör yoksa oluştur (Hazırlık)
if not os.path.exists(YENI_VERI_DIR):
    os.makedirs(YENI_VERI_DIR)

# AĞIRLIK KAYDIRMA KATSAYISI (Gerçek saha verisine ne kadar çok güveneceğiz?)
...
for tur in makineler:
    ...
    # 2. Ağırlıkları (Weights) Belirle - İŞTE SİHİR BURADA!
    # Eski verilere 1, yeni verilere 5 çarpanı veriyoruz.
    agirliklar_eski = np.full(len(df_eski), SENTETIK_VERI_AGIRLIGI)
    agirliklar_yeni = np.full(len(df_yeni), GERCEK_VERI_AGIRLIGI)
    
    # Verileri ve ağırlıkları birleştir (Harmanla)
    ...
    # 4. Yeni Modeli Eğit (Ağırlıklı Eğitim)
    ...
    # 5. Güncel Modeli Üzerine Yaz (Overwrite)
