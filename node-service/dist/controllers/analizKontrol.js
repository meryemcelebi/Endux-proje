...
exports.maliyetAnalizi = maliyetAnalizi;
exports.lokasyonHaritasi = lokasyonHaritasi;
const prisma_1 = __importDefault(require("../config/prisma"));
async function maliyetAnalizi(req, res) {
    try {
...
async function lokasyonHaritasi(req, res) {
    try {
        ...
        const haritaVerisi = makineler
            .filter((m) => {
            if (katFilter)
                return m.lokasyon.some((l) => l.kat === katFilter);
            return true;
        })
            .map((m) => {
            ...
            if (maliyetOrani > 10)
                renk = 'KIRMIZI';
            else if (maliyetOrani > 5)
                renk = 'SARI';
            else if (maliyetOrani >= 2)
                renk = 'TURUNCU';
            else
                renk = 'YESIL';
