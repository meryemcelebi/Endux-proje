        ...
        const [makineDurumlari, onayBekleyenMakineler, fabrikaOee, kritikRiskliMakineler, bakimiYaklasanAdaylar, maliyetAnalizi, makineBazliMaliyetler, parcaKategoriMaliyetleri, aylikMaliyetTrend] = await Promise.all([
            ...
            prisma_1.default.$queryRawUnsafe(`
                SELECT 
                    ...
                    (SELECT COALESCE(SUM(bakim_maliyet), 0)::FLOAT FROM bakim_kaydi bk 
                     ...
                     WHERE bt.bakim_tur_adi ILIKE '%Planlı%' OR bt.bakim_tur_adi ILIKE '%Önleyici%' OR bt.bakim_tur_adi ILIKE '%Periyodik%'
                        OR bk.aciklama ILIKE '%Planlı%' OR bk.aciklama ILIKE '%Önleyici%' OR bk.aciklama ILIKE '%Periyodik%') as planli_bakim_maliyeti,
                    
                    -- 2. Arızi Bakım Maliyeti (Geriye kalan tüm bakım maliyetleri)
                    (SELECT COALESCE(SUM(bakim_maliyet), 0)::FLOAT FROM bakim_kaydi bk 
                     LEFT JOIN bakim_turu bt ON bk.bakim_tur_id = bt.bakim_tur_id 
                     WHERE NOT (
                        COALESCE(bt.bakim_tur_adi, '') ILIKE '%Planlı%' OR COALESCE(bt.bakim_tur_adi, '') ILIKE '%Önleyici%' OR COALESCE(bt.bakim_tur_adi, '') ILIKE '%Periyodik%'
                        OR COALESCE(bk.aciklama, '') ILIKE '%Planlı%' OR COALESCE(bk.aciklama, '') ILIKE '%Önleyici%' OR COALESCE(bk.aciklama, '') ILIKE '%Periyodik%'
                     )) as arizi_bakim_maliyeti,
            ...
            prisma_1.default.$queryRawUnsafe(`
                SELECT 
                    ...
                    l.fabrika_alani,
                    -- Planlı Bakım
                    COALESCE(SUM(CASE WHEN bt.bakim_tur_adi ILIKE '%Planlı%' OR bt.bakim_tur_adi ILIKE '%Önleyici%' OR bt.bakim_tur_adi ILIKE '%Periyodik%' OR bk.aciklama ILIKE '%Planlı%' OR bk.aciklama ILIKE '%Önleyici%' OR bk.aciklama ILIKE '%Periyodik%' THEN bk.bakim_maliyet ELSE 0 END), 0)::FLOAT as planli_maliyet,
                    -- Arızi Bakım
                    COALESCE(SUM(CASE WHEN bk.bakim_id IS NOT NULL AND NOT (COALESCE(bt.bakim_tur_adi, '') ILIKE '%Planlı%' OR COALESCE(bt.bakim_tur_adi, '') ILIKE '%Önleyici%' OR COALESCE(bt.bakim_tur_adi, '') ILIKE '%Periyodik%' OR COALESCE(bk.aciklama, '') ILIKE '%Planlı%' OR COALESCE(bk.aciklama, '') ILIKE '%Önleyici%' OR COALESCE(bk.aciklama, '') ILIKE '%Periyodik%') THEN bk.bakim_maliyet ELSE 0 END), 0)::FLOAT as arizi_maliyet,
            ...
            prisma_1.default.$queryRawUnsafe(`
                ...
                bakim_maliyetleri AS (
                    SELECT 
                        ...
                        COALESCE(SUM(CASE WHEN bt.bakim_tur_adi ILIKE '%Planlı%' OR bt.bakim_tur_adi ILIKE '%Önleyici%' OR bt.bakim_tur_adi ILIKE '%Periyodik%' OR bk.aciklama ILIKE '%Planlı%' OR bk.aciklama ILIKE '%Önleyici%' OR bk.aciklama ILIKE '%Periyodik%' THEN bk.bakim_maliyet ELSE 0 END), 0)::FLOAT as planli,
                        COALESCE(SUM(CASE WHEN NOT (COALESCE(bt.bakim_tur_adi, '') ILIKE '%Planlı%' OR COALESCE(bt.bakim_tur_adi, '') ILIKE '%Önleyici%' OR COALESCE(bt.bakim_tur_adi, '') ILIKE '%Periyodik%' OR COALESCE(bk.aciklama, '') ILIKE '%Planlı%' OR COALESCE(bk.aciklama, '') ILIKE '%Önleyici%' OR COALESCE(bk.aciklama, '') ILIKE '%Periyodik%') THEN bk.bakim_maliyet ELSE 0 END), 0)::FLOAT as arizi,
        ...
        return res.status(200).json({
            ...
            data: {
                ...
                maliyet_ozeti: {
                    ...
                    aylik_trend: (aylikMaliyetTrend || []).map((row) => ({
                        ay: row.ay,
