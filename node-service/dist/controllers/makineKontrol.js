...
exports.makineDurumGuncelle = exports.qrileMakineGetir = exports.makineEkle = void 0;
exports.tumMakineBilgileriGetir = tumMakineBilgileriGetir;
exports.makineDetayGetir = makineDetayGetir;
exports.QRKodYazdir = QRKodYazdir;
const uuid_1 = require("uuid");
const prisma_1 = __importDefault(require("../config/prisma"));
const isDatabaseReachabilityError = (error) => ["P1001", "ETIMEDOUT", "ECONNREFUSED", "ENOTFOUND"].includes(error?.code) ||
    error?.message?.includes("Can't reach database server") ||
    error?.message?.includes("Connection terminated unexpectedly");
const databaseUnavailableMessage = "Veritabanına ulaşılamıyor. Yerel geliştirmede Docker/PostgreSQL servisinin çalıştığından emin olun.";
// Makine Ekle — POST /api/makineler/makine-ekle
const makineEkle = async (req, res) => {
    try {
        const { makine_adi, firma_id, makine_tur_id, seri_no, satin_alma_tarihi, satin_alma_maliyeti, aktiflik_durumu, makine_ozellikleri, garanti_firma_id, garanti_suresi, lokasyon_id, 
        // TPM: Yapılandırılmış teknik özellik alanları
        kapasite, guc_tuketimi, max_rpm, max_basinc_ton, enjeksiyon_hacmi, tabla_boyutu, guncel_calisma_saati, tedarikci } = req.body;
        const ozellikler = makine_ozellikleri;
        ...
        const yeniMakine = await prisma_1.default.makine.create({
            data: {
        ...
        let teknik_json = ozellikler ? { ...ozellikler } : {};
        if (kapasite)
            teknik_json.kapasite = String(kapasite);
        if (guc_tuketimi)
            teknik_json.guc_tuketimi = String(guc_tuketimi);
        if (max_rpm)
            teknik_json.max_rpm = Number(max_rpm);
        if (max_basinc_ton)
            teknik_json.max_basinc_ton = Number(max_basinc_ton);
        if (enjeksiyon_hacmi)
            teknik_json.enjeksiyon_hacmi = String(enjeksiyon_hacmi);
        if (tabla_boyutu)
            teknik_json.tabla_boyutu = String(tabla_boyutu);
        const ozellikData = {
            makine_id: yeniMakine.makine_id,
            teknik_ozellikler: Object.keys(teknik_json).length > 0 ? teknik_json : null,
        };
        await prisma_1.default.makine_ozellikleri.create({ data: ozellikData });
        // 4. Lokasyon bağlantısı varsa güncelle
        if (lokasyon_id) {
            let actual_lo_id = null;
            if (isNaN(Number(lokasyon_id))) {
                const foundLo = await prisma_1.default.lokasyon.findFirst({
                    where: { fabrika_alani: String(lokasyon_id) }
                });
                if (foundLo)
                    actual_lo_id = foundLo.lokasyon_id;
            }
            else {
                actual_lo_id = Number(lokasyon_id);
            }
            if (actual_lo_id) {
                await prisma_1.default.lokasyon.update({
        ...
        const finalMachine = await prisma_1.default.makine.findUnique({
            where: { makine_id: yeniMakine.makine_id },
            include: {
                firma: true,
                makine_turu: true,
...
exports.makineEkle = makineEkle;
// QR ile Makine Getir — GET /api/makineler/qr/:qr_uuid
const qrileMakineGetir = async (req, res) => {
    try {
...
exports.qrileMakineGetir = qrileMakineGetir;
async function tumMakineBilgileriGetir(req, res) {
    try {
...
;
async function makineDetayGetir(req, res) {
    try {
...
;
async function QRKodYazdir(req, res) {
    try {
...
const makineDurumGuncelle = async (req, res) => {
    try {
        const makine_id = parseInt(req.params.id);
        if (isNaN(makine_id)) {
        ...
        const sonuc = await prisma_1.default.makine.update({
            where: { makine_id },
        ...
        if (aktiflik_durumu === false) {
            const mevcutBekleyen = await prisma_1.default.bakim_kaydi.findFirst({
                where: {
                    makine_id,
            ...
            if (!mevcutBekleyen) {
                await prisma_1.default.bakim_kaydi.create({
                    data: {
                        makine_id,
                        bakim_maliyet: 0,
                        durum: 'BEKLEYEN',
...
exports.makineDurumGuncelle = makineDurumGuncelle;
