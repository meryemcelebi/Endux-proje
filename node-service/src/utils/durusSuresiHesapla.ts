...
interface VardiyaBilgisi {
  baslangic_saati: string; // "HH:mm"
  bitis_saati: string;     // "HH:mm"
}
 ...
 */
export function hesaplaDurusSuresi(
  baslangic: Date,
  bitis: Date,
  vardiyalar: VardiyaBilgisi[]
): number {
 ...
 */
export async function otomatikDurusSuresiHesapla(
  baslangic: Date,
  bitis: Date
): Promise<number> {
